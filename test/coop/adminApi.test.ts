...
describe("생협 Admin API", () => {
  ...
  it("시간표 전송 직전에 식사 타입과 금요일을 API 값으로 보정한다", async () => {
    const request = {
      coop_shops: [{
        coop_shop_info: {
          name: "학생식당",
        ...
        operation_hours: [
          ...
          { type: undefined, day_of_week: "FRIDAY", open_time: "휴점", close_time: "휴점" },
        ],
      }],
    };
    const fetchMock = vi.spyOn(globalThis, "fetch")
    ...
    expect(body.coop_shops[0].operation_hours.map((hour: { type: string }) => hour.type))
      .toEqual(["아침", "점심", "저녁", undefined]);
    expect(body.coop_shops[0].operation_hours.map((hour: { day_of_week: string }) => hour.day_of_week))
      .toEqual(["평일", "평일", "평일", "금요일"]);
    expect(request.coop_shops[0].operation_hours.map((hour) => hour.type))
      .toEqual(["조식", "중식", "석식", undefined]);
    expect(request.coop_shops[0].operation_hours[3].day_of_week).toBe("FRIDAY");
  });
