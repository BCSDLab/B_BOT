...
import { busActionValue, buildBusPatchBlocks } from "~/services/bus/slack";
import { downloadBusAttachment, findBusTimetableFile } from "~/services/bus/slackAttachment";
...
export const messages: MessageSetting[] = [
  {
    regex: COMMAND,
    acceptsFiles: true,
    async handler({ client, channel, ts, user, files }) {
      const file = findBusTimetableFile(files);
      if (!file) {
          ...
          blocks: [
            {
              ...
              text: {
                type: "mrkdwn",
                text: [":file_folder: 버스 시간표 파일을 함께 올려주세요.", "", USAGE].join("\n"),
              },
            },
          ],
        });
      ...
      try {
        const bytes = await downloadBusAttachment(file);
